...
class Formatter {
  ...
  formatDateTimeFromString(dt, fmt) {
    const knownEnglish = this.loc.listingMode() === 'en',
      ...
      meridiem = () => (knownEnglish ? meridiemForDateTime(dt) : string({ hour: 'numeric', hourCycle: 'h12' }, 'dayperiod')),
...
class DateTime {
  ...
  constructor(config) {
    ...
    let invalid =
      config.invalid || (Number.isNaN(config.ts) ? new Invalid('invalid input') : null) || (!zone.isValid ? unsupportedZone(zone) : null);
    /**
     * @access private
     */
  ...
  equals(other) {
    return (
      this.isValid && other.isValid && this.valueOf() === other.valueOf() && this.zone.equals(other.zone) && this.loc.equals(other.loc)
    );
  }
